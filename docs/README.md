...
- **[Install the CLIs](install.md)** — `dwx` and the `dwx mcp` MCP server via Homebrew, download, or source.
...
- **[Configuration](configuration.md)** — every environment variable and Helm value, plus metrics and the `dwx` health check.
